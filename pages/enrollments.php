...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_enrollment'])) {
    ...
    if ($student_id > 0 && $course_id > 0) {
        try {
            ...
            if ($e->getCode() == 23000) {
                set_flash_message('error', 'Student is already enrolled in this course.');
            } else {
                set_flash_message('error', 'Error enrolling student: ' . $e->getMessage());
            }
        }
    } else {
        set_flash_message('error', 'Please select both a student and a course.');
    }
...
// Total count
$totalStmt = $pdo->prepare("SELECT COUNT(*) FROM enrollments");
$totalStmt->execute();
...
$stmt = $pdo->prepare("
    SELECT e.id, e.enrollment_date, 
           s.first_name, s.last_name, s.department as student_dept,
...
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-2xl font-bold text-[#1a237e]">Course Enrollments</h1>
        <p class="text-sm text-gray-500">Manage student registrations</p>
    </div>
    <?php if(is_admin()): ?>
    ...
    <table class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                ...
                <th class="px-6 py-4">Student Dept</th>
            ...
            <?php else: ?>
                ...
                <tr class="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td class="px-6 py-4 text-gray-800">
                        ...
                        <?php echo htmlspecialchars($enr['course_name']); ?>
                    </td>
                    <td class="px-6 py-4">
                        <span class="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-xs font-bold"><?php echo htmlspecialchars($enr['student_dept']); ?></span>
                    </td>
                    <td class="px-6 py-4 text-gray-500 text-sm">
                        <?php echo date('d-m-Y h:i A', strtotime($enr['enrollment_date'])); ?>
                    </td>
                    <?php if(is_admin()): ?>
...
<div id="addModal" class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-[#1a237e]">Register Student to Course</h2>
        ...
        <form method="POST" action="">
            ...
            <div class="mb-6">
                <label class="block text-xs font-semibold text-gray-600 mb-1">Select Course</label>
                <select name="course_id" class="w-full px-3 py-2 border rounded focus:ring-1 focus:ring-[#1a237e]" required>
                    ...
                    <?php foreach($courses as $crs): ?>
                        <option value="<?php echo $crs['id']; ?>"><?php echo htmlspecialchars($crs['course_code'] . ' - ' . $crs['course_name']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
